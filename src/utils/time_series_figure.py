...
def make_traj_fig(
    ...
    if title is not None:
        ax.set_title(title)
    # ax setting
    ax.set_xlabel("$y_{\mathrm{E}} ~ \mathrm{[m]}$")
    ax.set_ylabel("$x_{\mathrm{E}} ~ \mathrm{[m]}$")
...
def make_ts_fig(ts_list, fig_size,):
    ...
    v_ax_range = drawing_range(ts_list, "psi [deg]", 0.05)
    ax_psi.set_xlim(time_min, time_max)
    ax_psi.set_ylim(v_ax_range[0], v_ax_range[1])
    for ts in ts_list:
        plot_ts(
            ax=ax_psi, ts=ts,
            df_label_t="t [s]", df_label_v="psi [deg]",
            fig_size=fig_size,
        )
    # speed
    ...
    v_ax_range = drawing_range(ts_list, "r [deg/s]", 0.05)
    ax_r.set_xlim(time_min, time_max)
    ax_r.set_ylim(v_ax_range[0], v_ax_range[1])
    for ts in ts_list:
        plot_ts(
            ax=ax_r, ts=ts,
            df_label_t="t [s]", df_label_v="r [deg/s]",
            fig_size=fig_size,
        )
    # ax_r.legend()
    #
...
def make_traj_and_velo_fig(ts_list, ship_plot_step_period, alpha_ship_shape, fig_size,):
    ...
    [time_min, time_max] = get_time_range(ts_list, "t [s]",)
    # traj
    ax_traj.set_xlabel("$y_{\mathrm{E}} ~ \mathrm{[m]}$")
    ax_traj.set_ylabel("$x_{\mathrm{E}} ~ \mathrm{[m]}$")
    L_max = max(ts.L for ts in ts_list)
    B_max = max(ts.B for ts in ts_list)
    h_ax_range, v_ax_range =  \
        drawing_range_2D(ts_list, "p_y [m]", "p_x [m]", L_max, B_max, True)
    ...
    for ts in ts_list:
        plot_traj(
            ax_traj, ts, "p_x [m]", "p_y [m]", "psi [rad]",
            ship_plot_step_period, alpha_ship_shape, ts.L, ts.B
        )
    # u
    # ax_speed.set_xlabel("$t$ [s]")
...
def plot_traj(
        ax, ts,
        x_column_label, y_column_label, psi_column_label,
        ...
        if ( j % ship_plot_step_period == 0 ) or ( j == len(ts.df) - 1 ):
            ...
            p = ts.df.iloc[
                ...
                [
                    ...
                    ts.df.columns.get_loc(psi_column_label),
                ]
            ]
            if j == 0:
                ax.add_patch(
